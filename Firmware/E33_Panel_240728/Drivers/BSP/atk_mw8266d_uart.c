...
static struct
{
    ...
    struct
    {
        ...
        uint16_t finsh  : 1;                                /* 帧接收完成标志，sta[15] */ //单位为位？
    } sta;                                                  /* 帧状态信息 */
...
/**
 * @brief       获取ATK-MW8266D UART接收到的一帧里所有数据
 * @param       无
...
{
    if (g_uart_rx_frame.sta.finsh == 1)//为1完成
    {
...
void ATK_MW8266D_UART_IRQHandler(void)
{
    ...
    if (__HAL_UART_GET_FLAG(&g_uart_handle, UART_FLAG_ORE) != RESET)        /* UART接收过载错误中断 */
    {
        ...
        (void)g_uart_handle.Instance->ISR;                                  /* 先读SR寄存器，再读DR寄存器 */
